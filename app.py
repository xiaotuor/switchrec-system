#!/usr/bin/env python3
# app.py · Streamlit 前端 (延迟 import Two-Tower)
...
def _init_all():
    ...
    uids = sorted(pd.read_csv(INTER_CSV)["user_id"].unique().tolist())
    return df, sim, uids
    ...
    for _, row in df_show.iterrows():
        ...
        with c2:
            ...
            if st.session_state.get(key):
                ...
                if st.button("加入收藏", key=key):
                    st.session_state.setdefault("favorites", []).append(row["name"])
...
with tab_tower:
    ...
    st.caption("基于训练好的用户/游戏向量召回 Top-N")

    # 延迟导入 tower_utils
    import tower_utils as tw

    uid_all = tw.get_all_user_ids()       # 取所有 user_id
    if "demo_uid" not in st.session_state:
        st.session_state["demo_uid"] = random.choice(uid_all)

    uid_sel = st.selectbox("选择 user_id", uid_all,
                           index=uid_all.index(st.session_state["demo_uid"]))
    if st.button("🔀 随机一个用户"):
        st.session_state["demo_uid"] = random.choice(uid_all)
        st.rerun()
...
fv = st.session_state.get("favorites", [])
if fv:
    fav_df = pd.DataFrame({"游戏名": sorted(set(fv))})
    st.sidebar.write(f"已收藏 {len(fav_df)} 款")
    ...
    st.sidebar.download_button("下载 CSV", buf.getvalue(), "my_favorites.csv", "text/csv")
else:
